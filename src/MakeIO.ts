...
export function MakeOutputIO(file: string | undefined): OutputIO {
  if (!file) {
    const res = (line: string) => new Promise<void>((resolve) => process.stdout.write(line, () => resolve())
    );
    res.close = () => Promise.resolve();
    return res;
  } else {
    const out = fs.createWriteStream(file);
    const res = (line: string) => new Promise<void>((resolve) => out.write(line, () => resolve()));
    res.close = () => new Promise<void>((resolve) => { out.on('close', resolve); out.close(); });
    return res;
  }
}
...
export function MakeIO(inPath: string | undefined, outPath: string | undefined): IOTriple {
